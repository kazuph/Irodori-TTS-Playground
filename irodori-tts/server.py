"""
FastAPI server wrapping Irodori-TTS InferenceRuntime.
POST /v1/tts  → WAV audio response
...
logger = logging.getLogger("irodori-tts")

app = FastAPI(title="irodori-tts")
...
def load_model():
    global runtime
    logger.info(f"Loading Irodori-TTS: {HF_CHECKPOINT}")
    ...
    logger.info("Irodori-TTS loaded")
...
def synthesize(req: TTSRequest):
    ...
    # Write WAV via temp file (torchaudio needs file extension)
    ...
    logger.info(
        f"TTS: {len(req.text)} chars -> {audio_duration:.1f}s audio in {elapsed:.2f}s "
...
if __name__ == "__main__":
    import uvicorn
    port = int(os.environ.get("IRODORI_PORT", "8005"))
